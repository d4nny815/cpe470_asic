...
module wptr_full #(parameter ADDR_SIZE = 4)(
    ...
    assign wbin_next = wbin + ({(ADDR_SIZE+1){winc & ~wfull}});
    // assign wbin_next = wbin + ({{ADDR_SIZE-2{1'd0}},winc} & ~{{ADDR_SIZE-2{1'd0}},wfull});       // Increment the write pointer if not full
